  ...
  function (Controller, MessageToast, MessageBox, JSONModel, Fragment) {
    ...
    return Controller.extend('com.study.controller.Books', {
      ...
      onOpenNotifications: function (oEvent) {
        ...
        this._pNotificationPopover.then(
          function (oPopover) {
      ...
      onMarkAllAsRead: function () {
        let oModel = this.getView().getModel();
        let aNotifications = this.getView()
          .getModel('notificationModel')
          .getProperty('/notifications');
        let aUnread = aNotifications.filter(function (n) {
          return !n.isRead;
        });
        ...
        aUnread.forEach(
          function (oNotification) {
            oModel.update(
              "/Notifications('" + oNotification.ID + "')",
              {
                isRead: true,
              },
              {
      ...
      onUploadTypeChange: function (oEvent) {
        ...
        switch (sKey) {
          case 'excel':
            ...
            oModel.setProperty(
              '/instructions',
              'Upload Excel file with columns: title, author_ID, price, stock'
            );
      ...
      _handleDocumentUpload: function (oFiles) {
        ...
        let fnProcessNextFile = function (index) {
          ...
          oModel.setProperty(
            '/uploadProgressText',
            Math.round(iProgressBase) + '% - Reading ' + oFile.name
          );
      ...
      _parsePDFFile: function (oFile, fnCallback) {
        ...
        if (typeof pdfjsLib === 'undefined') {
          MessageBox.warning(
            'PDF.js library not available. Cannot parse PDF files. Please use Excel format.'
          );
          fnCallback([]);
          return;
        }
      ...
      _parseWordFile: function (oFile, fnCallback) {
        ...
        if (typeof mammoth === 'undefined') {
          MessageBox.warning(
            'Mammoth library not available. Cannot parse Word files. Please use Excel format.'
          );
          fnCallback([]);
          return;
        }
      ...
      _extractBooksFromText: function (sText) {
        ...
        for (let k = iHeaderIndex + 1; k < aLines.length; k++) {
          ...
          if (Object.keys(oColumnIndices).length === 4) {
            ...
            oBook.author_ID = aParts[oColumnIndices.author]
              ? aParts[oColumnIndices.author].trim()
              : '';
      ...
      _handleImageUpload: function (oFiles) {
        ...
        let fnProcessNextFile = function (index) {
          ...
          oModel.setProperty(
            '/uploadProgressText',
            Math.round(iProgressBase) + '% - Processing ' + oFile.name
          );

          Tesseract.recognize(oFile, 'eng', {
            logger: function (m) {
      ...
      onFileSizeExceed: function (oEvent) {
        ...
        MessageBox.error("File '" + sFileName + "' exceeds maximum size of " + iMaxSize + 'MB');
      },
